...
async function handleProductFormSubmit(e) {
    ...
    try {
        ...
        saveButton.textContent = 'A analisar com IA...';
...
function openModal(productId = null) {
    ...
    modalContent.innerHTML = `
        <form id="product-form" class="p-8">
            ...
            <div class="space-y-4">
                <div><label for="name" class="block text-sm font-medium text-slate-700">Nome</label><input type="text" name="name" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm" value="${product.name || ''}"></div>
                <div><div class="flex justify-between items-center"><label for="description" class="block text-sm font-medium text-slate-700">Descrição</label><button type="button" id="generate-description-ai" class="text-xs bg-indigo-100 text-indigo-700 font-semibold px-2 py-1 rounded-md hover:bg-indigo-200">Gerar com IA</button></div><textarea name="description" rows="3" required class="mt-1 block w-full rounded-md border-slate-300 shadow-sm">${product.description || ''}</textarea></div>
...
function openCustomerModal(customerId) {
    ...
    const customerOrders = localOrders.filter(o => o.customer.id === customerId);
        ...
        <div class="p-8">
            <div class="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Cliente desde: ${formatDate(customer.createdAt)}</p>
                </div>
                <button type="button" data-action="close-modal" class="text-slate-400 hover:text-slate-600 text-3xl leading-none">&times;</button>
            </div>
